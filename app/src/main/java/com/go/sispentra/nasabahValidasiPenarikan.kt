...
class nasabahValidasiPenarikan : AppCompatActivity() {
    var baseUrl= BaseURL()
    //    ${baseUrl.url}
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.nasabah_validasi_penarikan)
    ...
    fun reqValidasiOrRejctPenarikan(loginData: LoginData,transaksi:Transaksi,validasi:Boolean=true,position:Int,mPenarikanAdapter:ValidasiPenarikanAdapter){
        ...
        if(validasi){
            URL +="validasi_nasabah/"+transaksi.id
        }
        else{
            URL +="reject_nasabah/"+transaksi.id
        }
        ...
        val jsonObject: JsonObjectRequest = object : JsonObjectRequest(
            ...
            Response.Listener { response ->
                ...
                if(validasi){
                    ...
                    Toast.makeText(this@nasabahValidasiPenarikan, "Validasi Berhasil", Toast.LENGTH_LONG).show()
                }
                else{
                    Toast.makeText(this@nasabahValidasiPenarikan, "Reject Berhasil", Toast.LENGTH_LONG).show()
                }
                try {
                ...
                if (error is TimeoutError || error is NoConnectionError || error is NetworkError) {
                    Toast.makeText(this@nasabahValidasiPenarikan, "Network Error", Toast.LENGTH_LONG).show()
                    ...
                    if(error.networkResponse.statusCode==401){
                        ...
                        editor.commit()
                        val intent = Intent(this@nasabahValidasiPenarikan, MainActivity::class.java)
                        startActivity(intent)
                        finish()
                    }
                    else if (error.networkResponse.statusCode==403){
                        Toast.makeText(this@nasabahValidasiPenarikan, "Forbiden", Toast.LENGTH_LONG).show()
                    }
                    else if (error.networkResponse.statusCode==422){
                        Toast.makeText(this@nasabahValidasiPenarikan, "Data Input Invalid", Toast.LENGTH_LONG).show()
                    }
                    else if (error.networkResponse.statusCode==404){
                        Toast.makeText(this@nasabahValidasiPenarikan, "Data Not Found", Toast.LENGTH_LONG).show()
                    }
                    else if (error.networkResponse.statusCode==400){
                        Toast.makeText(this@nasabahValidasiPenarikan, "Data Sudah Terubah Sebelumnya", Toast.LENGTH_LONG).show()
                    }
                    try {
                        mPenarikanAdapter.transaksis.removeAt(position)
                        mPenarikanAdapter.notifyItemRemoved(position)
...
//                    Toast.makeText(this@ketuaDataStaffActivity, "Server Error", Toast.LENGTH_LONG).show()
                    ...
                    Toast.makeText(this@nasabahValidasiPenarikan, "Parse Error", Toast.LENGTH_LONG).show()
        ...
        val jsonObjectArray: JsonArrayRequest = object : JsonArrayRequest(
            ...
            Response.Listener { response ->
                ...
                if(!response.isNull(0)){
                    ...
                    Toast.makeText(this@nasabahValidasiPenarikan, "Data Kosong", Toast.LENGTH_LONG).show()
                }
