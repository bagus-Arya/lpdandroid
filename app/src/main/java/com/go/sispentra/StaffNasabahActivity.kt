...
class StaffNasabahActivity : AppCompatActivity() {
    var baseUrl= BaseURL()
    //    ${baseUrl.url}
    ...
    private var kolektor_id:Int=0

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_staff_nasabah)
        if (this.intent.extras != null && this.intent.extras!!.containsKey("staff_id")) {
//            Toast.makeText(this@nasabahTabunganActivity, this.intent.extras!!.get("message").toString(), Toast.LENGTH_SHORT).show()
            kolektor_id=this.intent.extras!!.get("staff_id").toString().toInt()
        }
...
//                    val intent = Intent(this@StaffNasabahActivity, KolektorNasabahTabungan::class.java)
//                    startActivity(intent)
    ...
    fun reqGetNasabah(loginData: LoginData, URL:String,searchParam:String?){
        ...
        if(searchParam!=null){
            ...
            SURL=SURL+"&staff_id="+kolektor_id
        }
        else{
            SURL=SURL+"?staff_id="+kolektor_id
        }
...
//                        staffs=createArrayStaffs(response)
                    ...
                    Toast.makeText(this@StaffNasabahActivity, "Data Kosong", Toast.LENGTH_LONG).show()
                }
                refreshLayout.setRefreshing(false)
            }, Response.ErrorListener { error ->
                if (error is TimeoutError || error is NoConnectionError || error is NetworkError) {
                    Toast.makeText(this@StaffNasabahActivity, "Network Error", Toast.LENGTH_LONG).show()
                    ...
                    if(error.networkResponse.statusCode==401){
                        ...
                        editor.commit()
                        val intent = Intent(this@StaffNasabahActivity, MainActivity::class.java)
                        startActivity(intent)
                        finish()
                    }
                    else if (error.networkResponse.statusCode==403){
                        Toast.makeText(this@StaffNasabahActivity, "Forbiden", Toast.LENGTH_LONG).show()
                    }
                    else if (error.networkResponse.statusCode==422){
                        Toast.makeText(this@StaffNasabahActivity, "Data Input Invalid", Toast.LENGTH_LONG).show()
                    }
                    else if (error.networkResponse.statusCode==404){
                        Toast.makeText(this@StaffNasabahActivity, "Data Not Found", Toast.LENGTH_LONG).show()
                    }
//                    Toast.makeText(this@ketuaDataStaffActivity, "Server Error", Toast.LENGTH_LONG).show()
                    Log.d("httpfail13", error.toString())
                } else if (error is ParseError) {
                    Toast.makeText(this@StaffNasabahActivity, "Parse Error", Toast.LENGTH_LONG).show()
...
//                        window.setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_NOTHING);
        ...
        if (this.intent.extras != null && this.intent.extras!!.containsKey("message")) {
            Toast.makeText(this@StaffNasabahActivity, this.intent.extras!!.get("message").toString(), Toast.LENGTH_LONG).show()
        }
    }
