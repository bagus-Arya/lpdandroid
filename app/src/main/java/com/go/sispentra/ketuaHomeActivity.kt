        ...
        val jsonObjectRequest: JsonObjectRequest = object : JsonObjectRequest(
            ...
            Response.Listener { response ->
                ...
                try {
                    ...
                    val intent = Intent(this@ketuaHomeActivity, MainActivity::class.java)
                    startActivity(intent)
                    finish()
                ...
                if (error is TimeoutError || error is NoConnectionError) {
                    Toast.makeText(this@ketuaHomeActivity, "Network Error", Toast.LENGTH_LONG).show()
                    ...
                    if(error.networkResponse.statusCode==401){
                        ...
                        Toast.makeText(this@ketuaHomeActivity, "Forbiden", Toast.LENGTH_LONG).show()
                    }
                } else if (error is ServerError) {
                    Toast.makeText(this@ketuaHomeActivity, "Server Error", Toast.LENGTH_LONG).show()
                    Log.d("httpfail13", error.toString())
                } else if (error is NetworkError) {
                    Toast.makeText(this@ketuaHomeActivity, "Network Error", Toast.LENGTH_LONG).show()
                    Log.d("httpfail14", error.toString())
                } else if (error is ParseError) {
                    Toast.makeText(this@ketuaHomeActivity, "Parse Error", Toast.LENGTH_LONG).show()
