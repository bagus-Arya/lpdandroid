...
class ketuaTambahDataStaffActivity : AppCompatActivity() {
    ...
    fun getViewTextProfile():Staff{
        val staff_textfield_editor_username=findViewById<TextInputEditText>(R.id.staff_textfield_editor_username)
        ...
        var DataProfileStaff=Staff(staff_textfield_editor_nama.text.toString(),autoCompleteTxtjenisRole.text.toString(),autoCompleteTxtJenisKelamin.text.toString(),staff_textfield_editor_telepon.text.toString(),staff_textfield_editor_password.text.toString(),-1,staff_textfield_editor_username.text.toString())
    ...
    fun reqPostStaff(loginData: LoginData, URL:String,DataProfileStaff: Staff){
...
//                Toast.makeText(this@ketuaTambahDataStaffActivity, "User di tambah", Toast.LENGTH_LONG).show()
                val intent = Intent(this@ketuaTambahDataStaffActivity, ketuaDataStaffActivity::class.java)
                intent.putExtra("message","Staff Berhasil Ditambahkan")
                ...
                if (error is TimeoutError || error is NoConnectionError || error is NetworkError) {
                    Toast.makeText(this@ketuaTambahDataStaffActivity, "Network Error", Toast.LENGTH_LONG).show()
                    ...
                    if(error.networkResponse.statusCode==401){
                        ...
                        editor.commit()
                        val intent = Intent(this@ketuaTambahDataStaffActivity, MainActivity::class.java)
                        startActivity(intent)
                        finish()
                    }
                } else if (error is ServerError) {
                    if (error.networkResponse.statusCode==403){
                        Toast.makeText(this@ketuaTambahDataStaffActivity, "Forbiden", Toast.LENGTH_LONG).show()
                    }
                        ...
                        Toast.makeText(this@ketuaTambahDataStaffActivity,respone, Toast.LENGTH_LONG).show()
                    }
                    else if (error.networkResponse.statusCode==404){
                        Toast.makeText(this@ketuaTambahDataStaffActivity, "Data Not Found", Toast.LENGTH_LONG).show()
                    }
                    Log.d("httpfail13", error.toString())
                } else if (error is ParseError) {
                    Toast.makeText(this@ketuaTambahDataStaffActivity, "Parse Error", Toast.LENGTH_LONG).show()
...
//            Toast.makeText(applicationContext, "Item: $position", Toast.LENGTH_SHORT).show()
        ...
        btnTmb.setOnClickListener{
            reqPostStaff(loginData,postStaffURL,getViewTextProfile())
