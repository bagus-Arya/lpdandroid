...
class ValidasiPenarikanBendaharaActivity : AppCompatActivity() {
    var baseUrl= BaseURL()
    //    ${baseUrl.url}
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.bendahara_validasi_penarikan)
        ...
        val jsonObject: JsonObjectRequest = object : JsonObjectRequest(
            ...
            Response.Listener { response ->
                ...
                if(validasi){
                    ...
                    Toast.makeText(this@ValidasiPenarikanBendaharaActivity, "Validasi Berhasil", Toast.LENGTH_LONG).show()
                }
                else{
                    Toast.makeText(this@ValidasiPenarikanBendaharaActivity, "Reject Berhasil", Toast.LENGTH_LONG).show()
                }
                try {
                ...
                if (error is TimeoutError || error is NoConnectionError || error is NetworkError) {
                    Toast.makeText(this@ValidasiPenarikanBendaharaActivity, "Network Error", Toast.LENGTH_LONG).show()
                    ...
                    if(error.networkResponse.statusCode==401){
                        ...
                        editor.commit()
                        val intent = Intent(this@ValidasiPenarikanBendaharaActivity, MainActivity::class.java)
                        startActivity(intent)
                        finish()
                    }
                    else if (error.networkResponse.statusCode==403){
                        Toast.makeText(this@ValidasiPenarikanBendaharaActivity, "Forbiden", Toast.LENGTH_LONG).show()
                    }
                    else if (error.networkResponse.statusCode==422){
                        Toast.makeText(this@ValidasiPenarikanBendaharaActivity, "Data Input Invalid", Toast.LENGTH_LONG).show()
                    }
                    else if (error.networkResponse.statusCode==404){
                        Toast.makeText(this@ValidasiPenarikanBendaharaActivity, "Data Not Found", Toast.LENGTH_LONG).show()
                    }
                    else if (error.networkResponse.statusCode==400){
                        Toast.makeText(this@ValidasiPenarikanBendaharaActivity, "Data Sudah Terubah Sebelumnya", Toast.LENGTH_LONG).show()
                    }
                    try {
                        mPenarikanAdapter.transaksis.removeAt(position)
                        mPenarikanAdapter.notifyItemRemoved(position)
...
//                    Toast.makeText(this@ketuaDataStaffActivity, "Server Error", Toast.LENGTH_LONG).show()
                    ...
                    Toast.makeText(this@ValidasiPenarikanBendaharaActivity, "Parse Error", Toast.LENGTH_LONG).show()
        ...
        val jsonObjectArray: JsonArrayRequest = object : JsonArrayRequest(
            ...
            Response.Listener { response ->
                ...
                if(!response.isNull(0)){
                    ...
                    Toast.makeText(this@ValidasiPenarikanBendaharaActivity, "Data Kosong", Toast.LENGTH_LONG).show()
                }
