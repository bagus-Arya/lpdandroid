...
<html lang="en">
  ...
  <body>
    ...
    <script>
        ...
        const basicUrl='{{ route('getLaporanSetoranData', ['token'=> $userLoginData->token]) }}'
        ...
        buttonDownload.addEventListener("click", ()=>{
            const selectedOption = inputKolektor.options[inputKolektor.selectedIndex];
            const value = selectedOption.value;
            livesdownload='{{ route('downloadSetoran', ['token'=> $userLoginData->token]) }}'+'?id_kolektor='+value+'&start_date='+startDate.value+'&end_date='+endDate.value;
            ...
            .then(html => {
                ...
                var worker = html2pdf().set({filename:'laporanp-setoran.pdf'}).from(containerParser).outputPdf().then((pdf) => {
