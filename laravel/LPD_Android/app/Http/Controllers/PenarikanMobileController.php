...
class PenarikanMobileController extends Controller
{
    ...
    public function index(Request $request,$token){
        if($request->get('login_user')->role=="Bendahara"){
            $transaksis=Transaksi::where('type_transaksi','Penarikan')->where('status','unvalidated')->with('bukutabungan.nasabah.kolektor',function ($q){
                $q->withTrashed();
            })->get();
        ...
        elseif($request->get('login_user')->role=="Kolektor"){
            $transaksis=Transaksi::whereHas('bukutabungan',function($q) use($request){
    ...
    public function store(Request $request,$token){
        ...
        // check penarikan exist
        $countPenarikanNasabah=Transaksi::whereHas('bukutabungan.nasabah',function($q) use($request){
            $q->where('id',$request->get('login_user')->id);
        })->where('type_transaksi','Penarikan')->whereNot('status','like','rejected%')->whereNot('status','validated-nasabah')->count();

        if($countPenarikanNasabah>0){
            return response()->json(['message' => 'Penarikan Masih Berlangsung'], 400);
        }
